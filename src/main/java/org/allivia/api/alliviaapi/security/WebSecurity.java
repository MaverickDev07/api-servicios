...
import static org.allivia.api.alliviaapi.security.Constants.LOGIN_URL;
...
public class WebSecurity extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity httpSecurity) throws Exception {
		...
		httpSecurity
			...
			.cors().and()
			.csrf().disable()
			.authorizeRequests().antMatchers(HttpMethod.POST,"/api"+ LOGIN_URL).permitAll()
			.antMatchers(HttpMethod.POST,"/api/users/find").permitAll()
			.antMatchers("/**").permitAll()
			.anyRequest().authenticated().and()
				.addFilter(new JWTAuthenticationFilter(authenticationManager(), userRepository, iSuscripcionRepository, iControlSuscripcionServicioRepository, iPacienteRepository,iDoctorRepository,iEspecialidadDoctorRepository,iEspecialidadesRepository,iAfiliacionesRepository,iAfiliacionesDoctorRepository, iHoraAtencionRepository,iDiasRepository,iHorasRepository,iSuscripcionPacienteRepository));
